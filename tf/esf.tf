...
resource "aws_s3_bucket_acl" "bucket_acl" {
  bucket = aws_s3_bucket.esf-config-bucket[0] 
  acl    = "private"
}

resource "aws_s3_bucket_logging" "bucket_logging" {
  bucket = aws_s3_bucket.esf-config-bucket[0]  # The bucket you want to log (source bucket)

  target_bucket = data.aws_s3_bucket.s3_logging_bucket.id 

  target_prefix = "s3/${aws_s3_bucket.esf-config-bucket[0]}/" 
}
