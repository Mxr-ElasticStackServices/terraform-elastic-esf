locals {
  ...
  # role_name   = local.create_role ? coalesce(var.service-shortname-var.environment-var.role_name, var.function_name, "*") : null
  role_name = local.create_role ? "${var.service-shortname}-${var.environment}-${coalesce(var.role_name, var.function_name, "*")}" : null
